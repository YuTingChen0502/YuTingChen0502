# Hi, I'm Austin Chen

- CS Junior @ NYCU
- Research Assistant @ NCHC

## Education

- NYCU CSIE, Junior
- GPA: 3.81/4.30
- TOEIC: 935/990

## Experience

- Research Assistant, National Center for High-performance Computing (NCHC) — Sep 2026–Present
- Summer Intern, National Center for High-performance Computing (NCHC) — Jul–Aug 2026
- Undergraduate Researcher, Reinforcement Learning and Bandits Lab (REAL), NYCU — Sep 2026–Present
- HiPAC26 / APAC26 — HPC performance engineering with Quantum ESPRESSO and OpenFOAM

## What I am working on

- Test-time reinforcement learning — undergraduate research at REAL under Prof. Ping-Chun Hsieh
- Neural Pre-decoding in QEC — robustness under distribution shift, model adaptation, and GPU inference on NVIDIA H200
- OpenFOAM — MPI scaling, NUMA/rank placement, profiling, and source/runtime optimization for a ~65M-cell CFD workload

## Research

- Neural pre-decoding for quantum error correction — NCHC  
  Studied distribution shift and adaptation in neural QEC decoders

- Test-time reinforcement learning — REAL, NYCU  
  Ongoing undergraduate research on learning and adaptation at test time

## Repositories

- [hipac26-qe](https://github.com/YuTingChen0502/hipac26-qe) — Quantum ESPRESSO GPU performance engineering on NVIDIA H200
- [ai-eeg-dialogue-classification](https://github.com/YuTingChen0502/ai-eeg-dialogue-classification) — cross-subject EEG classification and transformer-based dialogue-continuity modeling
- Project Distort — C++ / OpenGL procedural coordinate-transformation engine with real-time GPU deformation and puzzle modes
- Secure Server Application — ECDHE + AES-GCM encrypted data distribution with single-use API keys
- [Aurea](https://github.com/YuTingChen0502/Aurea) — local-first travel workspace built with React, TypeScript, RxDB, and Supabase
- [isc26-dftbplus](https://github.com/YuTingChen0502/isc26-dftbplus) — ISC26 DFTB+ working copy with an experimental ELSI/OpenMP modification

chenyuting0502@gmail.com
